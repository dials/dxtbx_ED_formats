...
class FormatFalconIIRaw(Format):
  ...
  def understand(image_file):
    ...
    with open(image_file, 'rb') as f:
      header = f.read(177)
      data=f.read()
      if os.fstat(f.fileno()).st_size != 8388785: return False

    if header != ('FEI RawImage\x00\x01\x00\x00\x00\x00\x08\x00\x00\x00\x08'
  ...
  def _start(self):
    ...
    self._header_bytes = FormatFalconIIRaw.open_file(
