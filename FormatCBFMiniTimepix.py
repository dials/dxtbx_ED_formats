...
class FormatCBFMiniTimepix(FormatCBFMini):
    ...
    constructing a model for the experiment from this. This version handles
    a single panel without excluded pixels. Derived classes handle more complex
    cases."""
    ...
    def _scan(self):
        ...
        # Work around
        # self._cif_header_dictionary["Start_angle"]
        # self._cif_header_dictionary["Angle_increment"]
    ...
    def _detector(self):
        ...
        # set panel extent in pixel numbers and x, y mm shifts. For the single
        # panel case this is trivial
        pnl_data = []
        nx, ny = self._array_size
        pnl_data.append(
            {"xmin": 0, "ymin": 0, "xmax": nx, "ymax": ny, "xmin_mm": 0, "ymin_mm": 0}
        )
...
class FormatCBFMiniTimepix512(FormatCBFMiniTimepix):
    """Special case of FormatCBFMiniTimepix in which there is a four panel quad,
    with the large edge pixels left as they are in the array and not split into
    three pixels. These large pixels are excluded from the detector extent
    and the gain of the detector is exactly 1."""
...
class FormatCBFMiniTimepix1032(FormatCBFMiniTimepix):
    """Special version of FormatCBFMiniTimepix in which a quad of quads is
    represented as four panels, with each consisting of a 516 pixel quad with
    a gain of exactly 3"""
    ...
    def understand(image_file):
        ...
        for record in FormatCBFMini.open_file(image_file, "rb"):
            ...
            if "--CIF-BINARY-FORMAT-SECTION--" in record:
                in_binary_format_section = True
            elif in_binary_format_section and record.startswith("X"):
                mime_header += record
            if in_binary_format_section and len(record.strip()) == 0:
                # http://sourceforge.net/apps/trac/cbflib/wiki/ARRAY_DATA%20Category
                #    In an imgCIF file, the encoded binary data begins after
