...
class FormatSMV_TVIPS(FormatSMVADSC):
  ...
  def understand(image_file):
    ...
    if sorted(header.keys()) != ['BEAM_CENTER_X', 'BEAM_CENTER_Y', 'BIN',
      'BYTE_ORDER', 'DATE', 'DETECTOR_SN', 'DIM', 'DISTANCE', 'HEADER_BYTES',
      'OSC_RANGE', 'OSC_START', 'PHI', 'PIXEL_SIZE', 'SIZE1', 'SIZE2', 'TIME',
      'TWOTHETA', 'TYPE', 'WAVELENGTH']: return False

    # Check a few values are as expected
    if header.get('DETECTOR_SN') != 'unknown': return False
    if header.get('BIN') != '2x2': return False
    if header.get('BYTE_ORDER') != 'little_endian': return False
    if header.get('HEADER_BYTES') != '512': return False
    if header.get('PIXEL_SIZE') != '0.0311999992': return False
    if header.get('SIZE1') != '2048': return False
    if header.get('SIZE2') != '2048': return False
    if header.get('TYPE') != 'unsigned_short': return False
  ...
  def _detector(self):
    ...
    d = self._detector_factory.simple('PAD', distance, (beam_y, beam_x), '+x', '-y',
        pixel_size, image_size, trusted_range, gain=gain)
